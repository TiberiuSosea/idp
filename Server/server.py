...
def CSP_RUN(input_file, output_file, time):

    global done_exit
    global INPUT_FILE, OUTPUT_FILE, ALLOWED_TIME, my_dict, Activities, Costs, Domain
...
def start_computation():
    global TEMP_PERSON_NAME, TEMP_PROGRAM_NAME, connection, cursor
    ...
    TEMP_PERSON_NAME = data[0][0]
    TEMP_PROGRAM_NAME = data[0][1]

    # RUN CSP HERE
    CSP_RUN("work_file.yml", 'work_result.yml', int(request.form['time']))
    ...
    args = (TEMP_PERSON_NAME, TEMP_PROGRAM_NAME, res)
